...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generator ontologii RDF z dokumentów w folderze.")
    parser.add_argument("input", help="Ścieżka do folderu z dokumentami (.doc, .docx, .pdf, .txt, .md)")
    parser.add_argument("--output", default="ontology.ttl", help="Plik wynikowy .ttl")
    parser.add_argument("--chunk_size", type=int, default=4000, help="Rozmiar chunku tekstu (znaki)")
    parser.add_argument("--overlap_size", type=int, default=500, help="Nakładka między chunkami (znaki)")
    parser.add_argument("--delay_between_chunks", type=float, default=2.0, help="Opóźnienie między chunkami (sekundy)")
    args = parser.parse_args()

    try:
        main(args.input, args.output, args.chunk_size, args.overlap_size, args.delay_between_chunks)
    except Exception as e:
        console.print(f"Błąd krytyczny: {e}")
